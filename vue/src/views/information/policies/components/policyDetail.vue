          ...
          <Col span="24">
            <span class="menu-btn">
              <div class="menu-btn">
                <span
                  v-if="
                    this.policy.status == EStatus.WaitingForApproval &&
                ...
                <span
                  v-if="
                    this.policy.status == EStatus.Approved &&
                    SessionStore.state.user.canWaitingFromDraft
                  "
                ...
                <Button
                  v-if="
                    this.policy.status == EStatus.Draft &&
                    SessionStore.state.user.canWaitingFromDraft
                  "
                ...
                <Button
                  v-if="
                    this.policy.status == EStatus.Return &&
    ...
    >
      ...
      >
        ...
        <div class="content-news">
          ...
          <div class="type-name">
            <span class="policy-type">{{ this.policy.typeName }}</span>
            <p style="color: #777; font-size: 14px; margin-right: 15px">
              {{
                (this.policy.modifiedDate
                  ? this.policy.modifiedDate
                  : this.policy.createDate) | moment
              }}
            </p>
          </div>
          <div class="box-img">
            <img :src="this.policy.image" alt="img" class="img-news" />
          </div>
          <div class="description" v-html="policy.description"></div>
          <div class="wrapper-favourite">
            <div class="d-flex favourite">
              <div>{{ policy.userLike.totalLike }} Thích</div>
              <div>{{ policy.totalComment }} bình luận</div>
            </div>
            <hr />
            <div
              class="d-flex"
      ...
      >
        ...
        <Row
          v-for="(policy, index) in allPolicies"
          :key="index"
          align-v="center"
        ...
        >
          ...
          >
            <NewsItem
              :data="policy"
              @click.native="openEventDetail(policy.id)"
            />
          </Col>
        </Row>
...
export default class EventDetail extends Vue {
  ...
  private entityName: string = "Event";
  ...
  private async actionLike() {
    if(this.policy.userLike.isLiked){
      this.policy.userLike.isLiked =false;
    }else{
      this.policy.userLike.isLiked =true;
    }
  ...
  private async getTotalLike() {
    let data = await this.$store
      ...
      .catch(() => {});
    this.policy.userLike.isLiked = data.isLiked;
    this.policy.userLike.totalLike = data.totalLike;
  }
  ...
  async fetchData() {
    ...
    if (this.$route.params.policyId) {
      this.policy = await this.$store.dispatch({
        type: "policy/getPolicyById",
        id: this.$route.params.policyId,
      });
      this.policy.image = url + this.policy.image;
    }
    // this.viewFace();
  }
  // async viewFace() {
  ...
  async changeStatus(status) {
    let policyData = this.policy;
    policyData.status = status;
    policyData.coverImage = "";
    policyData.image = "";
    let formData = new FormData();
    for (let key in policyData) {
      if (key != "groupId") formData.append(key, policyData[key]);
      else {
        policyData.groupId.map((group) => {
  ...
  async getComment(status?: String) {
    ...
    let params = {
      entityId: this.$route.params.policyId,
      entityName: "Policy",
      pageNumber: 1,
  ...
  private openEditComment(item: any, isSub?: string) {
    ...
    this.entityId = this.$route.params.policyId;
    this.$forceUpdate();
  }
