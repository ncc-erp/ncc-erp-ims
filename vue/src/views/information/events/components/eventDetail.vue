    ...
    >
      ...
      >
        ...
        <div class="content-news">
         ...
         <div class="type-name">
            <span class="event-type">{{ this.event.typeName }}</span>
            <p style="color: #777; font-size: 14px; margin-right: 15px">
              {{
                (this.event.modifiedDate
                  ? this.event.modifiedDate
                  : this.event.createDate) | moment
              }}
            </p>
          </div>
          <div class="box-img">
            <img :src="this.event.image" alt="img" class="img-news" />
          </div>
          <div class="description" v-html="event.description"></div>
          <div class="wrapper-favourite">
            <div class="d-flex favourite">
              <div>{{ event.userLike.totalLike }} Thích</div>
              <div>{{ event.totalComment }} bình luận</div>
            </div>
            <hr />
            <div
              class="d-flex"
                  ...
                  ><Col span="23" class="box-content">
                    <div>
                      ...
                      <Mentionable
                        ...
                        :limit="5"
                        :mapInsert="mapInsert"
                        :class="{ boxReply : item.subComment, nBoxReply: !item.subComment}"
                        offset="6"
                      >
                        <textarea
                  ...
                  <Col span="22" class="box-reply">
                    <div>
                      ...
                      <Mentionable
                        ...
                        :limit="5"
                        :mapInsert="mapInsert"
                        :class="{ boxReply : element.subComment, nBoxReply: !element.subComment}"
                        offset="6"
                      >
                        <textarea
      ...
      <h3 class="relation">Tin liên quan</h3>
        ...
        >
          ...
          <Col
            ...
            :lg="24"
          >
            <NewsItem :data="event" @click.native="openEventDetail(event.id)" />
          </Col>
        </Row>
...
export default class EventDetail extends Vue {
  ...
  private async actionLike() {
     if(this.event.userLike.isLiked){
      this.event.userLike.isLiked =false;
    }else{
      this.event.userLike.isLiked =true;
    }
  ...
  private async getTotalLike() {
    let data = await this.$store
      ...
      .catch(() => {});
    this.event.userLike.isLiked = data.isLiked;
    this.event.userLike.totalLike = data.totalLike;
  }
  ...
  async fetchData() {
    ...
    if (this.$route.params.eventId) {
      ...
      this.event.image = url + this.event.image;
    }
    // this.viewFace();
  }
  // async viewFace() {
  ...
  async changeStatus(status) {
    ...
    for (let key in eventData) {
      if (key != "groupId") formData.append(key, eventData[key]);
      else {
        eventData.groupId.map((group) => {
  ...
  async getComment(status?: String) {
    ...
    let params = {
      entityId: this.$route.params.eventId,
      entityName: "Event",
      pageNumber: 1,
  ...
  private openEditComment(item: any, isSub?: string) {
    ...
    this.entityId = this.$route.params.eventId;
    this.$forceUpdate();
  }
