          ...
          <Col span="24">
            <span class="menu-btn">
              <div class="menu-btn">
                <span
                  v-if="
                    this.guide.status == EStatus.WaitingForApproval &&
                ...
                <span
                  v-if="
                    this.guide.status == EStatus.Approved &&
                    SessionStore.state.user.canWaitingFromDraft
                  "
                ...
                <Button
                  v-if="
                    this.guide.status == EStatus.Draft &&
                    SessionStore.state.user.canWaitingFromDraft
                  "
                ...
                <Button
                  v-if="
                    this.guide.status == EStatus.Return &&
                ...
                <Button
                  ...
                  v-if="
                    this.guide.canEdit ||
                    SessionStore.state.user.role == 'Admin'
                  "
                  @click="handleEditNew"
      ...
      >
        ...
        <div class="content-news">
          ...
          <!-- <div class="type-name">
            <p class="time-create">
              {{
                (this.guide.modifiedDate
                  ? this.guide.modifiedDate
                  : this.guide.createDate) | moment
              }}
            </p>
          </div> -->

              <div class="type-name">
            <span class="news-type">{{ this.guide.typeName }}</span>
          ...
          <div class="box-img">
            <img :src="this.guide.image" alt="img" class="img-news" />
          </div>
          <div class="description" v-html="guide.description"></div>
          <div class="wrapper-favourite">
            <div class="d-flex favourite">
              <div>{{ guide.userLike.totalLike }} Thích</div>
              <div>{{ guide.totalComment }} bình luận</div>
            </div>
            <hr />
            <div
              class="d-flex"
      ...
      <h3 class="relation">Tin liên quan</h3>
        ...
        >
          ...
          <Col
            ...
            :lg="24"
          >
            <NewsItem :data="guide" @click.native="openEventDetail(guide.id)" />
          </Col>
        </Row>
...
export default class GuideDetail extends Vue {
  ...
  private entityName: string = "GuildLine";
  ...
  private async actionLike() {
    if(this.guide.userLike.isLiked){
      this.guide.userLike.isLiked = false;
    }else{
      this.guide.userLike.isLiked = true;
    }
  ...
  private async getTotalLike() {
    let data = await this.$store
      ...
      .catch(() => {});
    this.guide.userLike.isLiked = data.isLiked;
    this.guide.userLike.totalLike = data.totalLike;
  }
  ...
  async fetchData() {
    ...
    if (this.$route.params.guideId) {
      this.guide = await this.$store.dispatch({
        type: "guide/getGuide",
        id: this.$route.params.guideId,
      });
      this.guide.image = url + this.guide.image;
    }
    // this.viewFace();
  }
  // async viewFace() {
  ...
  handleEditNew() {
    this.$router.push({
      name: "EditGuide",
      params: { guideId: this.$route.params.guideId },
    });
  }
  ...
  async changeStatus(status) {
    let guideData = this.guide;
    guideData.status = status;
    guideData.coverImage = "";
    guideData.image = "";
    let formData = new FormData();
    for (let key in guideData) {
      if (key != "groupId") formData.append(key, guideData[key]);
      else {
        guideData.groupId.map((group) => {
  ...
  async getComment(status?: String) {
    ...
    let params = {
      entityId: this.$route.params.guideId,
      entityName: "GuildLine",
      pageNumber: 1,
  ...
  private openEditComment(item: any, isSub?: string) {
    ...
    this.entityId = this.$route.params.guideId;
    this.$forceUpdate();
  }
