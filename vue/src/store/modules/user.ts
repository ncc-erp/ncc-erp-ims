...
class UserModule extends ListModule<UserState, any, User>{
    ...
    actions = {
        async getAll(context: ActionContext<UserState, any>, payload: any) {
            ...
            let reponse = await Ajax.get(`/api/services/app/User/GetAllPaging?isActive=${payload.data.isActive}&fd=${payload.data.fd}&td=${payload.data.td}`, { params: payload.data.value });
